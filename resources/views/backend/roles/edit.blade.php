        ...
        <div class="d-block mb-4 mb-md-0">
            <nav aria-label="breadcrumb" class="d-none d-md-inline-block">
                <ol class="breadcrumb breadcrumb-dark breadcrumb-transparent">
                    ...
                    <li class="breadcrumb-item active">{{ __('roles.edit_role.breadcrumb_edit') }}</li>
                </ol>
            </nav>

            <h2 class="h4">{{ __('roles.edit_role.title') }}</h2>
            <small class="mb-0">{{ __('roles.edit_role.description') }}</small>
        </div>

        <a href="{{ route('roles') }}" class="btn btn-gray-800 animate-up-2">
            <i class="fas fa-arrow-left me-2"></i> {{ __('roles.edit_role.back') }}
        </a>
    </div>
            ...
            <form action="{{ route('roles.update', $role->id) }}" method="POST">
                ...
                <div class="row">
                    <div class="col-md-4 mb-3">
                        <label class="form-label">{{ __('roles.edit_role.role_name') }}</label>
                        <input type="text" name="name" class="form-control"
                            placeholder="{{ __('roles.edit_role.role_name_placeholder') }}"
                            value="{{ old('name', $role->name) }}" required>
                    </div>

                    <div class="col-md-8 mb-3">
                        <label class="form-label">{{ __('roles.edit_role.permissions') }}</label>
                ...
                <button class="btn btn-primary mt-3 animate-up-2">
                    <i class="bi bi-save me-1"></i> {{ __('roles.edit_role.update_role') }}
                </button>
            </form>
        </div>
    </div>
